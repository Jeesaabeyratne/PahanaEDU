...
public class BillService {
    ...
    public List<BillDTO> getAllBills() {
        EntityManager em = emf.createEntityManager();
        try {
            List<Bill> bills = em.createQuery("SELECT b FROM Bill b ORDER BY b.billDateTime DESC", Bill.class)
                    .getResultList();
            return bills.stream()
                    .map(b -> new BillMapper().toDTO(b))
    ...
    // ---------------- VIEW BILLS BY CUSTOMER (Cashier) ----------------
    public List<BillDTO> getBillsByCustomer(int customerId) {
        EntityManager em = emf.createEntityManager();
        try {
            List<Bill> bills = em.createQuery(
                            "SELECT b FROM Bill b WHERE b.customer.id = :customerId ORDER BY b.billDateTime DESC",
                            Bill.class)
